...
// @return
func AppFollowUser(w *client.WeiboClient, wg *sync.WaitGroup) {
	if wg != nil {
		defer wg.Done()
	}
	uids := GetUids(w)
...
func GetUids(w *client.WeiboClient) (uids []string) {
...
// @param         wg         *sync.WaitGroup           等待组，保持程序同步
// @return
func AppFollowWithUndo(w *client.WeiboClient, uids []string, wg *sync.WaitGroup) {
	if wg != nil {
		defer wg.Done()
	}
