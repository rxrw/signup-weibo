...
// @return
func AppRepostCommentLike(w *client.WeiboClient, wg *sync.WaitGroup) {
	if wg != nil {
		defer wg.Done()
	}
	mids := GetMids(w)
...
func GetMids(w *client.WeiboClient) (mids []string) {
...
// @param         wg         *sync.WaitGroup           等待组，保持程序同步
// @return
func AppRepostWithDel(w *client.WeiboClient, mids []string, wg *sync.WaitGroup) {
	if wg != nil {
		defer wg.Done()
	}
...
// @param         wg         *sync.WaitGroup           等待组，保持程序同步
// @return
func AppCommentsWithDel(w *client.WeiboClient, mids []string, wg *sync.WaitGroup) {
	if wg != nil {
		defer wg.Done()
	}
	for ii, mid := range mids {
...
// @param         wg         *sync.WaitGroup           等待组，保持程序同步
// @return
func ApplikeWithUndo(w *client.WeiboClient, mids []string, wg *sync.WaitGroup) {
	if wg != nil {
		defer wg.Done()
	}
	for ii, mid := range mids {
